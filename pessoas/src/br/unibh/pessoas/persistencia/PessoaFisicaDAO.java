...
public class PessoaFisicaDAO implements DAO<PessoaFisica, Long> {
	...
	public PessoaFisica find(Long id) {
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement("select * from tb_pessoa_fisica "
				                                                                         + " where id =?");
			p.setLong(1, id);
	...
	public void insert(PessoaFisica t) {
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement("insert into tb_pessoa_fisica (nome, endereco, telefone, cpf, email, data_nascimento, sexo)" 
					                                                         +" values (?, ?, ?, ?, ?, ?, ?)");
	...
	public void update(PessoaFisica t) {
		try {
			PreparedStatement p = JDBCUtil.getConnection().prepareStatement("update tb_pessoa_fisica set nome=?, endereco=?, telefone=?,"
	...
	public List<PessoaFisica> findAll() {
		...
		try {
			...
			while(res.next()){
				lista.add(new PessoaFisica(res.getLong("id"), 
						res.getString("nome"), 
						res.getString("endereco"), 
						res.getString("telefone"), 
						res.getString("cpf"), 
